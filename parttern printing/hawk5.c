#include<stdio.h>  //To print holloww diamond pattern
int main(){        //using user input
  int n;
  printf("Enter the number of lines in odd number only:");
  scanf("%d",&n);//odd is for middle line
...
//    *
 ...
 int ml = n/2+1;//this is for middle line as odd number does not have a middle line so we are adding 1 and 7/2+1 = 4 is our middle line eg:- 3/2+1=2 i ml
  ...
  for(int i=1;i<=n;i++){ //for printing the lines only
    ...
    if(k == 1 || k==nst) //this will only print the outer stars 
      printf("*");
    else{ //if k is != 1 pr nst then in that place correspoding white space will be printed means instead of skipping it will print inside spaces
      printf(" ");
    }
    }
